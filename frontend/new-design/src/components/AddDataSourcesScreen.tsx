import { Wallet, TrendingUp, Share2, ChevronRight } from 'lucide-react';
import { Progress } from './ui/progress';

interface AddDataSourcesScreenProps {
  onSkip: () => void;
  onAddSource: (sourceId: string) => void;
}

export default function AddDataSourcesScreen({ onSkip, onAddSource }: AddDataSourcesScreenProps) {
  const dataSources = [
    {
      id: 'cex',
      label: 'CEX',
      description: 'Link your exchange account',
      icon: TrendingUp,
    },
    {
      id: 'crypto-wallet',
    ...
    },
    {
      id: 'socials',
      label: 'Link socials',
      description: 'For more secure access (in case u lose ur passkey)',
      icon: Share2,
    },
  ];
      ...
      <div className="w-full max-w-md mx-4 relative z-10">
        <div className="bg-white/70 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl shadow-green-500/10 p-8 h-[650px] flex flex-col">
          ...
          <div className="mt-auto space-y-6">
            <button
              onClick={onSkip}
              className="w-full bg-white/60 backdrop-blur-sm hover:bg-white/80 text-gray-700 p-4 rounded-xl border-2 border-gray-300/50 hover:border-gray-400 transition-all duration-300 hover:shadow-md"
            >
              Skip for now
            </button>
