...
const cryptoCards = [
  { id: 1, name: 'Ether.Fi' },
  { id: 2, name: 'Solayer card' },
  { id: 3, name: 'Phantom Card' },
  { id: 4, name: 'Neobankless' },
  { id: 5, name: 'Coinbase Card' },
  { id: 6, name: 'Crypto.com Card' },
  { id: 7, name: 'Nexo Card' },
  { id: 8, name: 'BlockCard' },
  { id: 9, name: 'Wirex Card' },
  { id: 10, name: 'BitPay Card' },
];

interface CryptoCardSearchProps {
  onBack: () => void;
  onCardSelect: (cardName: string) => void;
}

export default function CryptoCardSearch({ onBack, onCardSelect }: CryptoCardSearchProps) {
      ...
      <div className="w-full max-w-md mx-4 relative z-10">
        <div className="bg-white/70 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl shadow-green-500/10 p-8 h-[650px] flex flex-col">
          <div className="mb-6">
            ...
            <h1 className="text-gray-900 text-center">Select your crypto card</h1>
          </div>
              ...
              filteredCards.map((card) => (
                ...
                >
                  <div className="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-gray-100 to-gray-50 group-hover:from-green-50 group-hover:to-emerald-50 transition-all duration-300 shadow-sm">
                    <CreditCard className="w-5 h-5 text-gray-600 group-hover:text-green-600 transition-colors duration-300" />
                  </div>
                  <span className="flex-1 text-left">{card.name}</span>
                </button>
              ))
            ) : (
